...
__all__ = ['Toy_laz_dataset', 'MiniToy_laz_dataset']
...
def read_toy_laz_dataset_tile(
    ...
    instance: bool = False,
    remap: bool = False,
    #extra argument added for toy dataset
    rgb=False,
    max_intensity=600) -> Data:
    """Read a Toy_laz_dataset tile saved as .laz.
    ...
    :param remap: bool
        Whether semantic labels should be mapped from their toydataset ID
        to their train ID.
    """

    """Read a Toy_laz_dataset file saved as LAZ."""
    data = Data()
    key = 'testing'
    ...
    #if xyz:
    #    pos = torch.stack([
    #        torch.tensor(las[axis]) for axis in ["X", "Y", "Z"]
    #    ], dim=-1)
    #    pos *= las.header.scale
    #    pos_offset = pos[0]
    #    data.pos = (pos - pos_offset).float()
    ...
    if rgb:
        data.rgb = to_float_rgb(torch.stack([torch.from_numpy(np.array(las[axis],dtype=np.float32) / 65535) for axis in ["red", "green", "blue"] ], dim=-1))
    ...
    # Instance labels not supported in Vancouver dataset
    if instance:
...
class Toy_laz_dataset(BaseDataset):
    """Toy_laz_dataset dataset.
    ...
    def num_classes(self) -> int:
        ...
        indicated as `y=self.num_classes` in the dataset labels.
        """
        return TOY_DATASET_NUM_CLASSES
    ...
    def read_single_raw_cloud(self, raw_cloud_path: str) -> 'Data':
        ...
        """
        return read_toy_laz_dataset_tile(
...
class MiniToy_laz_dataset(Toy_laz_dataset):
    """A mini version of Toy_laz_dataset with only a few windows for
    experimentation.
    """
    _NUM_MINI = 2
